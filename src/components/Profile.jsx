...
export const Profile = ({ onNavigate, games, primaryPlayer, getPlayerStats, library = [] }) => {
  ...
  const displayedInsights = useMemo(() => {
    ...
    if (stats.competitiveGames > 0 && stats.cooperativeGames > 0) {
      ...
      if (diff > 15) pool.push(`⚔️ Você se sai melhor em competitivos (${stats.competitiveWinRate}%) do que em cooperativos (${stats.cooperativeWinRate}%)`);
      ...
      else pool.push(`⚖️ Desempenho equilibrado: ${stats.competitiveWinRate}% em competitivos e ${stats.cooperativeWinRate}% em cooperativos`);
    ...
    if (favCat) {
      ...
      pool.push(`🗂️ Sua categoria favorita é ${label} (${favCat[1]} partida${favCat[1] !== 1 ? 's' : ''})`);
    }
